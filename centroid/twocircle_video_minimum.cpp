...
//------------------------------	This default setting is for red color and variables
	...
	createTrackbar("LowH", "HSV Control", &iLowH, 179); //Hue (0 - 179)
	createTrackbar("HighH", "HSV Control", &iHighH, 179);
	createTrackbar("LowS", "HSV Control", &iLowS, 255); //Saturation (0 - 255)
	createTrackbar("HighS", "HSV Control", &iHighS, 255);
	createTrackbar("LowV", "HSV Control", &iLowV, 255);//Value (0 - 255)
	createTrackbar("HighV", "HSV Control", &iHighV, 255);
...
//----------------------------------	Read an image and check for invalid input
   int imgHeight = 480, //Default 480
	...
	CvSize imgSize = cvSize(imgWidth,imgHeight); // The saiz of the image used. Default capture size - 640x480.
...
while (true)
	{	//Grab image frame
        ...
        if (!bSuccess){ //if not success, break loop
			cout << "Cannot read a frame from video stream" << endl;getchar(); break;}  
