...
def OpenCV():
    ...
    while(1):
        ...
        thresh = cv2.inRange(hsv,np.array((5,167,159)), np.array((50,207,199)))#head
        thresh1 = cv2.inRange(hsv,np.array((77,103,169)), np.array((117,143,209))) #base
        thresh2 = thresh.copy()
        ...
        if iFrame >= 100:
            if cxAvg < (cx + 25) and cxAvg > (cx - 25):
                xOld == cxAvg
                stringXOk = "X Lock"
        ...
        if iFrame >= 100:
            print tX,tY
            if tX > cxAvg -75 and tX < cxAvg + 75:
                print "Made it through the X"
                if tY > cyAvg -95 and tY < cyAvg + 95:
        ...
        #Target circle
        cv2.circle(frame, (tX, tY), 10, (255, 0, 0), thickness=-1)
        cv2.circle(frame, (cx, cy), 10, (255, 0, 0), thickness=-1)
        #ser.write(botXY)
        ...
        #Target angle.
        cv2.line(frame, (tX,tY), (cxAvg,cyAvg),(0,255,0), 5)
        cv2.line(frame, (cx,cy), (cx1,cy1),(0,0,255), 5)
...
def whileloop():
      global tX  #<----I can't remember why...
      global cxAvg
      global cyAvg 
      global newTarget
      
      global tY #variable for maximum 
      if( tX > cxAvg -15 and tX < cxAvg + 15 and tY > cyAvg -15 and tY < cyAvg + 15):
                    newTarget = "Yes"
                    ser.write('s')
...
OpenCV.start()
whileloop=threading.Thread(target=whileloop)
whileloop.start()
